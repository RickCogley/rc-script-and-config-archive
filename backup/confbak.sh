...
# Setup vars 
dt=`date '+%Y-%m-%d_%H%M'`
filestoget="/etc /root /home/rcogley /var/spool/cron /var/lib/pgsql/data/postmaster* /var/lib/pgsql/data/*.conf /var/lib/pgsql/data/PG* /var/www/html/pga/conf/config.inc.php /opt/TrackStudio/etc /opt/TrackStudio/jetty /opt/TrackStudio/stopJetty /usr/lib/mailman/Mailman/mm* /rcutils/scripts/*"
archfile="etc.tar.gz"
...
HELP
	...
	(cd /;tar -czf "$targetdir/backup-$dt/$archfile" --exclude=/etc/.webmin-backup $filestoget)
...
/bin/tar -tf $targetdir/backup-$dt/$archfile | /usr/bin/mutt -s "$0 Backup $dt" -a $targetdir/backup-$dt/proc_info.txt report@esolia.net
