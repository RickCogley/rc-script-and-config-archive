...
do
	rm -f ${backupdir}/${MYSQLDB}-full.sql
	echo "Dumping ${MYSQLDB} ..."
	for TARGETTBL in $(echo "show tables" | $mysqlcmd $mysqluidpw ${MYSQLDB})
	do
		echo " --- ${TARGETTBL}"
		$mysqldumpcmd $mysqluidpw $mysqldumpoptions ${MYSQLDB} ${TARGETTBL} >> ${backupdir}/${MYSQLDB}-full.sql
	done
