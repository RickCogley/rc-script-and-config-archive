#!/bin/sh
#
#  FileMaker Server Backup Script ver.1.5.1
#  Backup FileMaker database files latest n hours and latest n days.
#  This script is written for FileMaker Server 7-10 on Mac OS X.
#
...
if [ -n "$ISCONSOLEARRIVE" ]; then
        ...
        echo "is fmsadmin dead?\n\n$ISCONSOLEARRIVE" | mail -s "`hostname` fmsadmin failure" takeuchi@nemoux.com,chobi-nyan@splash.jp
        exit 1
fi
echo "-------------------------------------------------------------------"
echo ""
...
if [ "$hour" -gt 0 ]; then
	cp -fp "$prefix"."$archive" "$hourlypath"/$hourlyfilename."$archive" 
fi
if [ "$day" -gt 0 ]; then
	cp -fp "$prefix"."$archive" "$dailypath"/$dailyfilename."$archive"
fi
if [ "$month" -gt 0 ]; then
	cp -fp "$prefix"."$archive"  "$monthlypath"/$monthlyfilename."$archive"
fi
echo "    ... Done!"
echo ""
#
...
## options for external volume ##
#
if [ "$ext_hourly" -eq 1 ] || [ "$ext_daily" -eq 1 ] || [ "$ext_monthly" -eq 1 ]; then
...
#	Syncronize each backup directory to alternate backup directory
	...
	if [ "$ext_daily" -eq 1 ]; then
		rsync -vrpo --delete -u "$dailypath" "$external"
	fi
	if [ "$ext_monthly" -eq 1 ]; then
		rsync -vrpo --delete -u "$monthlypath" "$external"
	fi
	currentnum=4
	echo "    ... Done!"
	echo ""
...
## options for alternate backup server 1 ##
#
if [ "$alt1_hourly" -eq 1 ] || [ "$alt1_daily" -eq 1 ] || [ "$alt1_monthly" -eq 1 ]; then
	echo `expr "$currentnum" \+ 1`".  Transfering archive files to alternate backup server 1";
#
#
#	Syncronize each backup directory to other host via network
	if [ "$alt1_hourly" -eq 1 ]; then
		rsync -vrpo --delete -u -e ssh "$hourlypath" "$host1user"@"$host1":"$host1path"/
	fi
	if [ "$alt1_daily" -eq 1 ]; then
		rsync -vrpo --delete -u -e ssh "$dailypath" "$host1user"@"$host1":"$host1path"/
	fi
	if [ "$alt1_monthly" -eq 1 ]; then
		rsync -vrpo --delete -u -e ssh "$monthlypath" "$host1user"@"$host1":"$host1path"/
	fi
echo "    ... Done!"
echo ""
echo ""
...
## options for alternate backup server 2 ##
#
if [ "$alt2_hourly" -eq 1 ] || [ "$alt2_daily" -eq 1 ] || [ "$alt2_monthly" -eq 1 ]; then
        echo `expr "$currentnum" \+ 1`".  Transfering archive files to alternate backup server 2";
#
#
#       Syncronize each backup directory to other host via network
        if [ "$alt2_hourly" -eq 1 ]; then
                rsync -vrpo --delete -u -e ssh "$hourlypath" "$host2user"@"$host2":"$host2path"/
        fi
        if [ "$alt2_daily" -eq 1 ]; then
                rsync -vrpo --delete -u -e ssh "$dailypath" "$host2user"@"$host2":"$host2path"/
        fi
        if [ "$alt2_monthly" -eq 1 ]; then
                rsync -vrpo --delete -u -e ssh "$monthlypath" "$host2user"@"$host2":"$host2path"/
        fi
echo "    ... Done!"
echo ""
echo ""
