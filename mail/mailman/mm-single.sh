...
echo "mm-single.sh - <\$Revision: 0.5 $>"
...
while [ "$x" != "y" ]; do
  ...
  read CSN
  echo Enter the name of the string to complete the list name. 
  echo   i.e. Support or P1 or Projects etc, capital letter first.
  read LISTSTR
  ...
  echo Client List String: $LISTSTR
  echo List to Create: $CSN-$LISTSTR
...
do 
    ...
    echo "autoresponse_postings_text = \"\"\"Thank you for your message to %(listname)s, which has just been received into our system. An eSolia 
professional on your engagement team will attend to it directly. " >>/tmp/mm-$CSN-$i
    echo >>/tmp/mm-$CSN-$i
...
echo
$MMBIN/list_lists |grep $CSN-$LISTSTR
